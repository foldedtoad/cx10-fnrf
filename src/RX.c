...
void getRXDatas()
{
    ...
    for (i = 0; i < 6; i++) {
        if (chanNewValue[chanOrder[i]] &&
            ((channelBuffer[i] - RawChannels[chanOrder[i]] > 1) ||
             (channelBuffer[i] - RawChannels[chanOrder[i]] < 1))) {
            if (i == 0) {
                RXcommands[i] = constrain(RawChannels[chanOrder[i]] - 1000, 0,
                                          1000);    // throttle
            } else {
                RXcommands[i] = constrain(RawChannels[chanOrder[i]] - 1500, -500, 500);
            }
